...
Start
 ...
 ;So, I will use Port E Pins 0 and 1, and Port F pin 4. Since I am not using Port F pin 0, I need not unlock anything.
	...
	MOV R3, #7;Frequency is 8 Hertz at first, an on the real board, 1000 Hz is actually 1191 Hz, so the real board is faster than the simulator. 8/1.191 = 6.717 (around 7)
...
Breathe
	...
	MOV DutyCycle, #0
	MOV Frequency, #55 ;Change frequency to 80 Hz.
	
Direction RN 1

	MOV Direction, #0

ButtonCheck	LDR R0, =GPIO_PORTF_DATA_R
	LDRB R0, [R0]
	AND R0, R0, #0x10 ;Preserve bit 4. This is a negative logic switch. 0 = closed, 1 = open
	CMP R0, #0	
...
doneBreathing	POP {R0, R1, R2, R3, R4, LR}
	BX LR
